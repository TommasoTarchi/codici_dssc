...
```{r}
par(mfrow=c(2,2))
fit.best <- lm(log(TPY) ~ log(NUMBED) + TAXEXEMPT + PREV_UTILIZATION_RATE, data = data_2001)
summary(fit.best)
plot(fit.best)
```
```{r}
par(mfrow=c(2,2))
fit.best <- lm(log(TPY) ~ log(SQRFOOT) + TAXEXEMPT, data = data_2001)
summary(fit.best)
plot(fit.best)
```
...
## Correlation among UTILIZATION_RATE from the two years

```{r}
mean(abs(data_2001$UTILIZATION_RATE - data_2001$PREV_UTILIZATION_RATE))
```


```{r}
data_2001$UR_cat <- (data_2001$UTILIZATION_RATE > 0.85 & data_2001$UTILIZATION_RATE < 1.07)
data_2001$PUR_cat <- (data_2001$PREV_UTILIZATION_RATE > 0.85 &
                        data_2001$PREV_UTILIZATION_RATE < 1.07)
data_2001$UR_cat <- as.factor(data_2001$UR_cat)
data_2001$PUR_cat <- as.factor(data_2001$PUR_cat)

levels(data_2001$UR_cat) <- c("out", "in")
levels(data_2001$PUR_cat) <- c("out", "in")
```
```{r}
barplot(prop.table(table(data_2001$UR_cat, data_2001$PUR_cat), margin = 1),
        beside=TRUE, xlab="UTILIZATION_RATE_2000",col=c("gray","black"))
legend("topleft", c("UTILIZATION_RATE_2001=out", "UTILIZATION_RATE_2001=in"), cex = 0.95, bty = "n",fill=c("gray","black"))
```
