...
Le diverse dimensioni dei due alberi ottenuti con tree() e rpart() sono dovute al fatto che la tecnica del recursive binary splitting ha bisogno di un parametro per lo stop. Evidentemente nei due pacchetti il valore di default di questo parametro è diverso.
...
Esistono pacchetti appositi per il calcolo dell'errore.
...
```
Notare che i due errori hanno una componente randomica.
...
error.sl
```
Errore assoluto nell'unità di misura di Sepal.Length.
