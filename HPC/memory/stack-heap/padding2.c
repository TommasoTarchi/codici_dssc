/* shows how structs' fields are aligned in stack (with padding);
 * if an argument (of any kind) is passed from command line then
 * also alignment for packed structurs is shown */

/* notice that structs are placed automatically in the most
 * convenient order (as any set of variables that are not grouped
 * in a struct) */

/* notice that there is padding also at the end of structs: it is
 * determined by the longest field in the struct (it is long a
