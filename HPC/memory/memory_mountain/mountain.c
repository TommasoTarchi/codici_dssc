...
/* $end mountainmain */
    ...
    FILE* dat;
    dat = fopen("mountain.dat", "w");
    fprintf(dat, "# Clock frequency is approx. %.1f MHz\n", Mhz);
    fprintf(dat, "# Memory mountain (MB/sec)\n");
	...
	printf("s%d\t", stride);
    */
    fprintf(dat, "%d\t", MAXSTRIDE);
    for (stride = 1; stride <= MAXSTRIDE; stride++)
      fprintf(dat, "%d\t", stride);
...
/* $end mountainmain */
      ...
      */
      ...
      fprintf(dat, "%d\t", log2size_kb);

/* $begin mountainmain */
	for (stride = 1; stride <= MAXSTRIDE; stride++) {
	    fprintf(dat, "%.0f\t", run(size, stride, Mhz));
