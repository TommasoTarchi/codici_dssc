/* here we give an example of false sharing (happening on the vector S) 
 *
 * a possible way to fix it (without changing to much the code, for example 
 * using reduction()) is to make S a matrix (instead of a vector) with lines 
 * longer than cache lines and as many lines as threads, and then to store 
 * in the first element of each line the summation performed by each thread */
