/* this code illustrates how to use the attribute copyprivate: allows to propagate
 * a private (any kind of private) from a thread changing it in a single region
 * (it can be used ONLY with single regions) to all the others */

/* NOTICE: you should NEVER use drand48() with openMP; there are different ways
 * of generating random numbers that work with openMP, but here we used drand48()
