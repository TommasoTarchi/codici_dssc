/* implements different ways to sort the elements of two arrays */

/* NOTICE: to run it do:
 * gcc -march=native -I/scratch/Software/include -DUSE_PAPI -o sort_arraysort_array
 * arrays.c -lm -L/scratch/Software/lib -lpapi; add -D... to indicate 
 * which way to implement and add -DDEBUG if you want debugging as well */

/* NOT WORKING: probably issues with PAPI installation */
...
int main(int argc, char **argv)
{
  ...
  for (int cc = 0; cc < iter; cc++)
	...
	PAPI_START_CNTR;
		...
		A[ii] = A[ii]^B[ii];
							    */
							    ...
							    /* this tries to avoid jump instr    */
                                         ...
                                         /* and optimized using the register keyword */
	    ...
	    A[ii] = AA^(x & t);   // this is weird, because (for what I know) negative numbers representation is not always the same
	    B[ii] = BB^(x & t);
	    ...
	    d &= (d >> 31);    // weird, because (for what I know) the behavior is undefined for negative numbers
	    A[ii] = AA - d; 
	    B[ii] = BB + d;
