/* to help the compiler to optimize producing the least amount of branches
 * possible, if-else constructs should always be avoided inside loops;
 * here some examples on how to avoid that: using functions pointers (in 
 * this case the compiler builds a static table of code pointers) and
 * translating the nested for loops with a switch construct (notice that
 * doing that is not always possible) */

/* NOTICE that a significant difference in performance can be seen only if 
 * the optimization is switched on (and that not in all combinations of
 * cases there is an actual improvement) */

/* remember to compile using the linker -lm (for the math library) */
...
int main(int argc, char **argv)
{
  ...
  /* this implementation rarely improves the performance; 
   * instead, it often make it much poorer; this could be 
   * due to the fact that the functions pointed here are 
   * very simple */
  else if ( mode == 1 )
    /*
     * FUNCTION POINTERS
     ...
     */
    {
      ...
      /* NOTICE the genius way to produce a integer that describes the
       * combinations of the three cases */
