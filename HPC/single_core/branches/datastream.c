/* this code generates a random vector and (AFTER SORTING) provides three
 * different ways to select only those elements that are larger than a
 * given pivot */

/* to properly run it you must define the macros BESMART or BESMART2
 * during compilation using the -d option, for example: gcc datastream.c
 * -DBESMART */

/* notice that the difference in performance can be seen only if optimization
 * is turned off (apparently the compiler already optimizes) */
        ...
        for (ii = 0; ii < SIZE; ii++)
          {
            ...
            // CHIARIRE CHE STA SUCCEDENDO QUI!
