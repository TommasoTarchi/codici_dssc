/* vectorized version of particles.c */
...
int main(int argc, char **argv)
{
  ...
  for( int p = 0; p < Np3; p += 12 )
	...
	double dx2[4] = {(parts[p] - jks[i])*(parts[p] - jks[i]),
       ...
       //#pragma ivdep    // tells the compiler to vectorize the loop ignoring possible dependencies
						// (the compiler is not allowed to vectorize the loop if it can prove that
						// each iteration is independent from the previous one; in this way we are
						// telling it to ignore this possibility)
	for( int j = 0; j < Ng; j++)
	  {
