/* in this program we present two functions (one is the optimized
 * version of the other) to perform matrix-matrix multiplication*/
...
/* here we keep c's elements fixed and compute them doing the scalar 
 * product between a's rows and b's columns (as you would do "on paper"):
 * we are accessing a by row and b by column (not very efficient) */
...
// optimized version
/* here, instead, we keep a's elements fixed and compute their
 * contributes to the elemnts of c: we are accessing both a and c by
 * row (very efficient) */
