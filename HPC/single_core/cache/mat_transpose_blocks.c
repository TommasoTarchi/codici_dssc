/* improved version of mat_transpose.c: matrices are not accessed by
 * rows/columns, but using blocks; of course this strategy is the best
 * when we know the cache size, so that we are able to choose a block
 * size such that the blocks fit in the cache*/

/* testing the program for different block sizes, we find out that the
 * best performance is achieved for block sizes in a range between around
 * 60 and 400; this could be quite coherent with the GUESSED size of the
 * caches (L1: 32kB, L2: 256kB, L3: 8192kB) */
