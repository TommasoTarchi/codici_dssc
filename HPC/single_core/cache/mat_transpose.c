/* NOT WORKING: probably because of some issue with PAPI installation */

/* this is an optimized version of a matrix transposition program; notice
 * that matrices are stored as 1D arrays, that the matrix accessed by row
 * is the original one and the one by column the new one (more efficient
 * access) and that unrolling was used for the transposition loop*/
	  ...
	  for (int i = 0; i < N; i++)
	    {
	      ...
	      for(j = 0; j < N4; j+=4)
		{
          // unroll the loop by 4 (actually it is more a pipeline optimization than a cache optimization)
